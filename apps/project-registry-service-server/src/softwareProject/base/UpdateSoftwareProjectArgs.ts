/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SoftwareProjectWhereUniqueInput } from "./SoftwareProjectWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { SoftwareProjectUpdateInput } from "./SoftwareProjectUpdateInput";

@ArgsType()
class UpdateSoftwareProjectArgs {
  @ApiProperty({
    required: true,
    type: () => SoftwareProjectWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => SoftwareProjectWhereUniqueInput)
  @Field(() => SoftwareProjectWhereUniqueInput, { nullable: false })
  where!: SoftwareProjectWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => SoftwareProjectUpdateInput,
  })
  @ValidateNested()
  @Type(() => SoftwareProjectUpdateInput)
  @Field(() => SoftwareProjectUpdateInput, { nullable: false })
  data!: SoftwareProjectUpdateInput;
}

export { UpdateSoftwareProjectArgs as UpdateSoftwareProjectArgs };
